...
import {
	...
	FindHookParams,
	InsertOneHookParams,
	InsertManyHookParams,
	UpdateOneHookParams,
	UpdateManyHookParams,
	DeleteOneHookParams,
...
export class DbContext {
	...
	private prepareSetUnsetUpdateOp<T>(obj: T, def: ModelDefinition<T>, out?: any, keyPrefix?: string) {
		...
		for (let i = 0; i < def.members.length; ++i) {
			...
			// update embedded child object fields in the case where
			// the whole object hasn't changed. We don't handle arrays of embedded objects here
			if (f.model && !f.model.id && !is_dirty && m[f.key] && !Array.isArray(m[f.key])) {
...
const ID_QUERY_OPS = ['$in', '$nin', '$ne', '$eq'];

function FormatQueryForCompareOps(value: any, id: ID) {

	if (typeof value === 'object') {
		for (let k of ID_QUERY_OPS) {
			if (value[k]) {
				if (Array.isArray(value[k])) {
					value[k] = value[k].map((v: any) => EnsureId(v, id));
				}
				else {
					value[k] = EnsureId(value[k], id);
				}
				return true;
			}
		}
...
function FormatQueryField(value: any, id: ID) {
	...
	if (!FormatQueryForCompareOps(value, id)) {
